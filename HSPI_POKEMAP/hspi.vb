...
Public Class HSPI
    ...
    Dim IFACE_NAME As String = "POKEMAP"
    Dim sConfigPage As String = IFACE_NAME
    ...
    Public Function PluginFunction(ByVal proc As String, ByVal parms() As Object) As Object Implements IPlugInAPI.PluginFunction
        Try
            ...
            If mi Is Nothing Then
                logutils.Log("Method " & proc & " does not exist in this plugin.", MessageType.Error_)
                Return Nothing
            End If
            Return (mi.Invoke(Me, parms))
        Catch ex As Exception
            logutils.Log("Error in PluginProc: " & ex.Message, MessageType.Error_)
        End Try
    ...
    Public Function PluginPropertyGet(ByVal proc As String, parms() As Object) As Object Implements IPlugInAPI.PluginPropertyGet
        Try
            ...
            If mi Is Nothing Then
                logutils.Log("Property " & proc & " does not exist in this plugin.", MessageType.Error_)
                Return Nothing
            End If
            Return mi.GetValue(Me, parms)
        Catch ex As Exception
            logutils.Log("Error in PluginPropertyGet: " & ex.Message, MessageType.Error_)
        End Try
    ...
    Public Sub PluginPropertySet(ByVal proc As String, value As Object) Implements IPlugInAPI.PluginPropertySet
        ...
        Catch ex As Exception
            logutils.Log("Error in PluginPropertySet: " & ex.Message, MessageType.Error_)
        End Try
    End Sub
    ...
    Public Function InitIO(ByVal port As String) As String Implements HomeSeerAPI.IPlugInAPI.InitIO
        Try
            hs = utils.hs
            logutils = New LogUtils(hs, IFACE_NAME)
            utils.RegisterWebPage(IFACE_NAME)
            utils.callback.RegisterEventCB(Enums.HSEvent.VALUE_CHANGE, IFACE_NAME, "")
    ...
    Public Sub ShutdownIO() Implements HomeSeerAPI.IPlugInAPI.ShutdownIO
        Try
            ...
            Catch ex As Exception
                logutils.Log("could not save devices", MessageType.Error_)
            End Try
            utils.bShutDown = True
        Catch ex As Exception
            logutils.Log("Error ending " & IFACE_NAME & " Plug-In", MessageType.Error_)
        End Try
    End Sub
    ...
    Public Sub HSEvent(ByVal EventType As Enums.HSEvent, ByVal parms() As Object) Implements IPlugInAPI.HSEvent
        ...
        Select Case EventType
            Case Enums.HSEvent.VALUE_CHANGE
                ...
                If (hs.GetINISetting("param", "ModuleReceiveSMS", "", utils.INIFILE) = parms(4)) Then
                    Dim dvReceiveSMS As DeviceClass = hs.GetDeviceByRef(parms(4))
                    Dim dvExpediteurSMS As DeviceClass = hs.GetDeviceByRef(hs.GetINISetting("param", "ModuleExpediteurSMS", "", utils.INIFILE))
                    gérerSMS(dvExpediteurSMS.devString(Nothing), dvReceiveSMS.devString(Nothing))
                End If
    ...
    Public Sub New()
        utils = New HSPIUtils.Outils(IFACE_NAME)
