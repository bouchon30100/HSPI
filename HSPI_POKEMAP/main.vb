...
TryAgain:
        Try
            ...
            utils.callback = clientCallback.ServiceProxy
            APIVersion = utils.callback.APIVersion  ' will cause an error if not really connected
        Catch ex As Exception
        ...
        Try
            ...
            utils.callback = utils.callback
            utils.hs = host
            ' connect to HS so it can register a callback to us
            host.Connect(utils.IFACE_NAME, "")
            Console.WriteLine("Connected, waiting to be initialized...")
            Do
                Threading.Thread.Sleep(30)
            Loop While client.CommunicationState = HSCF.Communication.Scs.Communication.CommunicationStates.Connected And Not utils.bShutDown
            If Not utils.bShutDown Then
                gAppAPI.ShutdownIO()
