Imports System.IO
Imports HomeSeerAPI.DeviceTypeInfo_m
...
#End If
    ...
    Public Sub createMultiModules(FileIni As String, ReCreate As Boolean)
        Dim housecodes() As String = hs.GetINISetting("COMMON", "DEVICES", "", FileIni).Split("|")
        For Each housecode As String In housecodes
            Path.GetFileName(FileIni)
            Dim dir As String = FileIni.Replace(Path.GetFileName(FileIni), "")
            '       createModule(m)
        Next

    End Sub
    Private Sub createModule(m As String, b As Boolean)

    End Sub

    Public Sub SaveCreateModule(m As Module1Wire)

        Dim housecode As String = m.HC & m.MC
        Dim adress As String = m.OneWireAdress & "/" & m.Channel
        Dim name As String = m.NAME
        Dim location As String = m.PIECE
        Dim location2 As String = m.ETAGE
        Dim TypeString As String = m.hsType
        Dim StatusSupport As Boolean = False
        Dim CanDim As Boolean = False
        Dim Nb_Status As Integer = 0
        Dim Nb_StatusGraph As Integer = 0
        ...
        Dim refDev = hs.DeviceExistsAddress(adress, True)
        ...
        'Dim i = 0
        'For i = 1 To Nb_Status
        '    CreateStatusPAIR(dv.Ref(Nothing), FileIni, i)

        'Next
        'i = 0
        'For i = 1 To Nb_StatusGraph
        '    CreateStatusGraphiquePAIR(dv.Ref(Nothing), FileIni, i)
        'Next
    ...
    Private Sub CreateStatusPAIR(device As Integer, FileIni As String, num_STATUS_int As Integer)

        Dim num_STATUS As String = num_STATUS_int.ToString()
        Dim Type_Status As String = hs.GetINISetting("STATUS" + num_STATUS, "TYPE_STATUS", "", FileIni)
        Dim Status_ControlString As String = hs.GetINISetting("STATUS" + num_STATUS, "STATUS_CONTROLE", "", FileIni)
        Dim Value As String = hs.GetINISetting("STATUS" + num_STATUS, "VALUE", "", FileIni)
        Dim Offset As String = hs.GetINISetting("STATUS" + num_STATUS, "OFFSET", "", FileIni)
        Dim Status As String = hs.GetINISetting("STATUS" + num_STATUS, "STATUS", "", FileIni)
        ...
        If (UCase(Type_Status) = "SINGLE") Then
            ...
            Pair = New HomeSeerAPI.VSPair(Status_control_texte_to_int(Status_ControlString))
    ...
    Private Sub CreateStatusGraphiquePAIR(device As Integer, FileIni As String, num_STATUS_int As Integer)

        Dim num_STATUS As String = num_STATUS_int.ToString()
        Dim Type_Status As String = hs.GetINISetting("GRAPHICS" + num_STATUS, "TYPE_STATUS", "", FileIni)
        Dim Value As String = hs.GetINISetting("GRAPHICS" + num_STATUS, "VALUE", "", FileIni)
        Dim ImageStatus As String = hs.GetINISetting("GRAPHICS" + num_STATUS, "IMAGE_STATUS", "", FileIni)
